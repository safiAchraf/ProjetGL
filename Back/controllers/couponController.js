...
const createNewCoupon = async (req, res) => {
  ...
  if (salon.ownerId !== ownerId) {
    return res.status(403).json({ error: "You are not authorized to create coupons for this salon" });
  }
...
const deleteCoupon = async (req, res) => {
  ...
  try {
    ...
    const [salon] = await prisma.$queryRaw`SELECT * FROM "Salon" WHERE id = ${coupon.salonId}`;
...
const getSalonCoupons = async (req, res) => {
  try {
    ...
    if(salon.ownerId !== req.user.id) {
      return res.status(403).json({ error: "You are notauthorized to get this salon's coupons" });
    }
...
const updateCoupon = async (req, res) => {
  ...
  try {
    ...
    if (coupon.ownerId !== ownerId) {
      return res.status(403).json({ error: "You are not authorized to update this coupon" });
    }
